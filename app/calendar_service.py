...
def _get_credentials() -> Credentials:
    if not GOOGLE_TOKEN_JSON:
        raise RuntimeError("GOOGLE_TOKEN_JSON is not set. Run scripts/generate_token.py first.")

    creds = Credentials(
        token=GOOGLE_TOKEN_JSON.get("token"),
        refresh_token=GOOGLE_TOKEN_JSON.get("refresh_token"),
        token_uri="https://oauth2.googleapis.com/token",
        client_id=GOOGLE_CREDENTIALS_JSON.get("installed", GOOGLE_CREDENTIALS_JSON.get("web", {})).get("client_id"),
        client_secret=GOOGLE_CREDENTIALS_JSON.get("installed", GOOGLE_CREDENTIALS_JSON.get("web", {})).get("client_secret"),
        scopes=SCOPES,
    )
...
def get_free_days(start_date: str, end_date: str) -> list:
    """
    Returns a list of full calendar days (YYYY-MM-DD) within [start_date, end_date]
    that have no events booked at all.
    """
